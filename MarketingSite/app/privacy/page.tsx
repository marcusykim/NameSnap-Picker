  ...
  return (
    <main className="public-page"><SiteHeader />
      ...
      <article className="policy policy-grid shell">
        ...
        <section><h2>Web session and purchase data</h2><p>NameSnap Web stores one strictly necessary, randomly generated browser identifier in local browser storage. Free use does not require an account. Before a web purchase, Firebase Authentication sends a passwordless sign-in link that verifies the purchase email. Cloudflare stores only one-way hashes of the browser identifier, Firebase user identifier, and verified email together with the selected plan, entitlement status, and Stripe customer or subscription identifiers. This lets NameSnap block duplicate checkout and restore a purchase on another browser without storing your contestant list. Stripe processes web payments and may collect an email address, billing details, and payment information under Stripe’s privacy policy. NameSnap does not receive your full payment-card number.</p></section>
        ...
        <section><h2>Service providers and data sharing</h2><p>Firebase provides website hosting and passwordless purchase-account authentication. Cloudflare provides the payment service endpoint and stores the minimal web entitlement record described above. Stripe processes web purchases. Apple processes in-app purchases. NameSnap does not sell or rent personal data and does not share it with advertisers or data brokers.</p></section>
